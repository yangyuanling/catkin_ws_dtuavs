...
namespace linktrack
{
  ...
  {
    ros::NodeHandle n("~");
    //n.getParam("RecvDataMsgTopic",RecvDataMsgTopic);//
    n.getParam("SendDataMsgTopic",SendDataMsgTopic);//
    n.getParam("UwbInforMsgTopic",NlinkLinktrackNodeFrameTopic);//
    n.getParam("RecvDataMsgTopic",NlinkLinktrackNodeframe0Topic);//
   // uwb0_data_pub = nh_.advertise<nlink_parser::LinktrackNode0>(RecvDataMsgTopic,1);//
  ...
  {
    dt_sub_ =
        nh_.subscribe(SendDataMsgTopic, 1000, DTCallback);
  }
  ...
  void Init::initNodeFrame0(NProtocolExtracter *protocol_extraction)
  {
    ...
    protocol->SetHandleDataCallback([=] {
      if (!publishers_[protocol])
      {
        auto topic = NlinkLinktrackNodeframe0Topic.c_str();//
        publishers_[protocol] =
      ...
      {
       // nlink_parser::LinktrackNode0 pubMsg;//
       // pubMsg.role = data.role;
      //  pubMsg.id   = data.id;
       ...
       // std::cout<<msg_node<<std::endl;//
        stringstream sstrm;
        sstrm << node->data;
        sstrm >> msg_node.data;
        //sstrm >> pubMsg.data;
       // pubMsg.data = sstrm.str();
       // memcpy(msg_node.data, node->data, node->data_length);
       //
       //  uwb0_data_pub.publish(pubMsg);
      }
  ...
  void Init::initNodeFrame2(NProtocolExtracter *protocol_extraction)
  {
    ...
    protocol->SetHandleDataCallback([=] {
      if (!publishers_[protocol])
      {
        auto topic = NlinkLinktrackNodeFrameTopic.c_str();//
        publishers_[protocol] =
