...
dt_network_client::dt_network_client(int argc, char **argv)
{
  ...
  nh.param("source_id",_sourceID,1);
  nh.param("target_id",_targetID,-1);
  ...
  ros_pub = nh.advertise<dt_message_package::CloudMessage>(ros_pub_topic,1);
  ...
  else
  {
    ...
    {
      DTUAV::HeartMsg initMsg;
      initMsg.is_get = true;
      std::string msgData = x2struct::X::tojson(initMsg);

      DTUAV::socket_data_packet_stamp data_packet;
      data_packet.MessageData = msgData;
      data_packet.MessageID   = HeartMsgID;
      data_packet.SourceID    = _sourceID;
      data_packet.TargetID    = _targetID;
      data_packet.TimeStamp   = ros::Time::now().toNSec();
      ...
      if(buf[0] == '\n'&&buf[1]=='\n')
      {
        ...
        //  std::cout<<"packet_data_size:"<<packet_data_size<<std::endl;
          ...
          if(recv(connfd,buff,packet_data_size,0) == packet_data_size)
          {
            ...
            if(is_load)
            {
              dt_message_package::CloudMessage data_packet_msg;
...
void dt_network_client::rosSubMsgCallback(const dt_message_package::CloudMessageConstPtr &msg)
{
  DTUAV::socket_data_packet_stamp data_packet;
